...
func countTokens(request executorRequest) (any, *nativeabi.Error) {
	enc, err := TokenizerForModel(request.Model)
	if err != nil {
		return nil, failure(fmt.Errorf("commandcode tokenizer init: %w",
			err))
	}
	count, err := CountOpenAIChatTokens(enc, request.Payload)
	if err != nil {
		return nil, failure(fmt.Errorf("commandcode token counting: %w", err))
	}
	return executorResponse{Payload: BuildOpenAIUsageJSON(count)}, nil
}
